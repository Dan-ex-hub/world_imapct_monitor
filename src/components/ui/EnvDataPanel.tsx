...
import { Wind, Droplets, Thermometer, Activity, Flame, CloudRain, Waves } from 'lucide-react'
...
export function EnvDataPanel() {
  ...
  const renderLayerStats = () => {
    switch (activeEnvLayer) {
      case 'wind':
        ...
        const avgWindSpeed =
          envLayerData.wind.reduce((sum, p) => sum + p.speed, 0) / envLayerData.wind.length
        ...
        return (
          <div className="space-y-4">
            <StatCard label="Global Avg Wind Speed" value={`${avgWindSpeed.toFixed(1)} m/s`} />
            <div>
      ...
      case 'aqi':
        ...
        const topPolluted = [...envLayerData.aqi].sort((a, b) => b.aqi - a.aqi).slice(0, 5)

        return (
          <div className="space-y-4">
            <StatCard label="Monitoring Stations" value={envLayerData.aqi.length.toString()} />
            <div>
              <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-text-muted">
              ...
              <div className="space-y-2">
                {topPolluted.map((point, idx) => (
                  ...
                  >
                    <div>
                      <div className="text-sm font-medium text-text-primary">{point.city}</div>
                      <div className="text-xs text-text-muted">{point.country}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-semibold text-env-aqi">{point.aqi}</div>
                      <div className="text-xs text-text-muted">{point.category}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      case 'earthquakes':
        if (!envLayerData.earthquakes || envLayerData.earthquakes.length === 0) {
        ...
        return (
          <div className="space-y-4">
            <StatCard label="Last 24 Hours" value={envLayerData.earthquakes.length.toString()} />
            <div>
              <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-text-muted">
                Recent Earthquakes
              </h4>
                ...
                {topQuakes.map((quake) => (
                  <div key={quake.id} className="rounded-md bg-bg-card p-2">
                    <div className="mb-1 flex items-center justify-between">
                      ...
                      <span className="text-xs text-text-muted">{quake.depth.toFixed(0)} km deep</span>
                    </div>
                    <div className="text-xs text-text-secondary">{quake.location}</div>
      ...
      case 'wildfires':
        ...
        return (
          <div className="space-y-4">
            <StatCard label="Active Fires" value={envLayerData.wildfires.length.toString()} />
            <div>
              <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-text-muted">
                Recent Fires
              </h4>
              <div className="space-y-2">
                {envLayerData.wildfires.slice(0, 10).map((fire) => (
                  <div key={fire.id} className="rounded-md bg-bg-card p-2">
                    <div className="text-sm font-medium text-text-primary">{fire.title}</div>
                    <div className="mt-1 text-xs text-text-muted">
                      {formatDistanceToNow(new Date(fire.date), { addSuffix: true })}
                    </div>
                  </div>
                ))}
              </div>
      ...
      case 'storms':
        ...
        return (
          <div className="space-y-4">
            <StatCard label="Active Storms" value={envLayerData.storms.length.toString()} />
            <div>
              <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-text-muted">
                Storm List
              </h4>
              <div className="space-y-2">
                {envLayerData.storms.map((storm) => (
                  <div key={storm.id} className="rounded-md bg-bg-card p-2">
                    <div className="text-sm font-medium text-text-primary">{storm.title}</div>
                    {storm.category && (
                      <div className="mt-1 text-xs text-env-storm">{storm.category}</div>
                    )}
                    <div className="mt-1 text-xs text-text-muted">
                      {formatDistanceToNow(new Date(storm.date), { addSuffix: true })}
                    </div>
                  </div>
                ))}
              </div>
      ...
      case 'temperature_anomaly':
        if (!envLayerData.tempAnomalies || envLayerData.tempAnomalies.length === 0) {
          return <EmptyState message="No temperature anomaly data" />
        }
        const avgAnomaly =
        ...
        return (
          <div className="space-y-4">
            <StatCard
              label="Global Avg Anomaly"
              value={`${avgAnomaly > 0 ? '+' : ''}${avgAnomaly.toFixed(2)}°C`}
            />
          </div>
        )

      case 'sea_temp':
        return <EmptyState message="Sea temperature data will be available in Phase 6" />
  ...
  return (
    <div className="fixed right-0 top-16 z-30 h-[calc(100vh-64px)] w-80 border-l border-border-subtle bg-bg-surface/80 backdrop-blur-sm">
      ...
      <div className="border-b border-border-subtle p-4">
        ...
        <div className="mt-2 flex items-center justify-between text-xs text-text-muted">
          ...
          {envLayerData.updatedAt && (
            <span>
              Updated {formatDistanceToNow(new Date(envLayerData.updatedAt), { addSuffix: true })}
            </span>
          )}
        </div>
      </div>
      ...
      {/* Content */}
      <div className="overflow-y-auto p-4" style={{ height: 'calc(100% - 89px)' }}>
        {renderLayerStats()}
      </div>
    </div>
  )
